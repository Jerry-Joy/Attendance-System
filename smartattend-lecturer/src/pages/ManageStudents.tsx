    ...
    <div className="p-5 lg:p-8 max-w-5xl mx-auto animate-slide-up">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleCopy}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-brand-50 hover:bg-brand-100 text-brand-600 rounded-xl text-sm font-medium transition-colors"
          >
            {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
            {copied ? 'Copied!' : 'Copy'}
          </button>
          <button className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-brand-50 hover:bg-brand-100 text-brand-600 rounded-xl text-sm font-medium transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {studentList.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-slate-400 text-xs mt-1">Share the course code to get started</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-5 py-3 hidden sm:table-cell">Index Number</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-5 py-3 hidden md:table-cell">Joined</th>
                  <th className="text-center text-xs font-semibold text-slate-500 uppercase tracking-wider px-5 py-3">Attendance</th>
                  ...
                  <tr key={student.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-brand-50 flex items-center justify-center text-brand-600 font-semibold text-xs shrink-0">
                          {student.avatarInitials}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-slate-800">{student.name}</p>
                          <p className="text-xs text-slate-500 sm:hidden">{student.indexNumber}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5 hidden md:table-cell">
                      <span className="text-sm text-slate-500">{student.joinedDate}</span>
                    </td>

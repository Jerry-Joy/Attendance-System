  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900">
      ...
      >
        ...
        <button
          onClick={() => setSidebarOpen(false)}
          className="lg:hidden absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className={`${collapsed ? 'px-3' : 'px-5'} py-5 border-b border-slate-100 dark:border-slate-700 transition-all duration-300`}>
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
            <div className={`flex items-center ${collapsed ? '' : 'gap-3'}`}>
              ...
              {!collapsed && (
                <div className="overflow-hidden">
                  <h1 className="text-slate-800 dark:text-white font-bold text-lg tracking-tight whitespace-nowrap">SmartAttend</h1>
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="hidden lg:flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
        ...
        <nav className={`flex-1 ${collapsed ? 'px-2' : 'px-3'} py-4 space-y-1 transition-all duration-300`}>
          {navItems.map((item) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <item.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'text-brand-600 dark:text-brand-400' : ''}`} />
        ...
        <div className={`${collapsed ? 'px-2' : 'px-3'} py-3 border-t border-slate-100 dark:border-slate-700 transition-all duration-300`}>
          ...
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3 px-3'} py-2 mb-1`}>
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group relative"
            ...
            {!collapsed && (
              <span className="text-sm text-slate-500 dark:text-slate-400">{isDark ? 'Dark Mode' : 'Light Mode'}</span>
            )}
          </div>
          ...
          {collapsed && (
            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-center mt-2 p-2 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors group relative"
              title="Logout"
            >
              <LogOut className="w-4 h-4" />
              <span className="absolute left-full ml-3 px-2.5 py-1.5 bg-slate-800 dark:bg-slate-600 text-white text-xs font-medium rounded-lg whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none shadow-lg z-[60]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="hidden lg:flex items-center justify-between px-8 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500">
            <span>Portal</span>
            <span>/</span>
            <span className="text-slate-700 dark:text-slate-200 font-medium">Lecturer Dashboard</span>
          </div>
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-brand-500 flex items-center justify-center shadow-md shadow-brand-200 dark:shadow-brand-500/20">
              <GraduationCap className="w-4 h-4 text-white" />
            </div>
            <span className="font-bold text-slate-800 dark:text-white text-sm">SmartAttend</span>
          </div>
          <button
            onClick={toggleTheme}
            className="p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"

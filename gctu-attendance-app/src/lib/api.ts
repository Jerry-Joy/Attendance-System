...
export interface BackendCourse {
  ...
  endTime: string | null;
  lecturerId: string;
  lecturer?: { fullName: string } | string; // May be returned by backend as string or related object
  createdAt: string;
  updatedAt: string;
...
export function mapCourse(c: BackendCourse): MappedCourse {
  ...
  if (typeof c.lecturer === 'string') {
    lecturerName = c.lecturer;
  } else if (c.lecturer && typeof c.lecturer === 'object' && c.lecturer.fullName) {
...
export interface MappedLiveSession {
  sessionId: string;
  courseId: string;
  courseCode: string;
  courseName: string;
...
export function mapLiveSession(s: BackendActiveSession): MappedLiveSession {
  return {
...
export const api = {
  ...
  // Sessions (student)
  getActiveSessions: () =>
    request<BackendActiveSession[]>('/sessions/active'),

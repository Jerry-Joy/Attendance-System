...
import { History as HistoryIcon, UserCircle, QrCode, CheckCircle2, XCircle, Calendar, Clock, TrendingUp, Filter as FilterIcon, Bell } from 'lucide-react-native';
                ...
                <View className="p-4 gap-2.5">
                  ...
                  <View className="flex-row justify-between items-start">
                    <View className="flex-1">
                      ...
                      <Text className="text-lg font-bold text-primary leading-tight">{record.courseName}</Text>
                      <Text className="text-sm font-semibold mt-0.5" style={{ color: '#F5B41C' }}>{record.courseCode}</Text>
                    </View>
                  </View>

                  {/* Course details */}
                  <View className="flex-row items-center gap-2 bg-surface px-3 py-2 rounded-lg border border-outline-variant">
                    <UserCircle size={16} color="#64748B" />
                    <Text className="text-xs text-on-surface-variant font-medium flex-1" numberOfLines={1}>{record.lecturer}</Text>
                  </View>

                  {/* Bottom section */}
                  <View className="flex-row justify-between items-center pt-2.5 border-t border-outline">
                    <View className="flex-row items-center gap-2">
                      <View className="flex-row items-center gap-1 bg-surface-container px-2.5 py-1.5 rounded-lg">
                        <Calendar size={14} color="#64748B" />
                        <Text className="text-xs text-on-surface-variant font-semibold">{record.date}</Text>
                      </View>
                      <View className="flex-row items-center gap-1 bg-primary/10 px-2.5 py-1.5 rounded-lg">
                        <Clock size={14} color="#081637" />
                        <Text className="text-xs text-primary font-bold">{record.time}</Text>
                      </View>
                    </View>
                    {record.method && (
                      <View className="flex-row items-center gap-1 px-2.5 py-1.5 rounded-lg" style={{ backgroundColor: '#F5B41C' }}>
                        <QrCode size={12} color="#081637" strokeWidth={2.5} />

  ...
  return (
    <div className="p-5 lg:p-8 max-w-5xl mx-auto animate-slide-up">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-800 dark:text-white">Real-Time Attendance</h1>
              {activeSession && (
                <div className="flex items-center gap-1.5 px-2.5 py-0.5 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold">Total Count</span>
          </div>
          <div className="flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-brand-600 dark:text-brand-400">{totalCount}</span>
            <span className="text-sm text-slate-400 dark:text-slate-500">/ {enrolled}</span>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold">GPS Verified</span>
          </div>
          <span className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{gpsVerified}</span>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex items-center gap-2 mb-2">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold">Live Count</span>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="flex gap-2">
          {(
            ...
            >
              {f.label}
              <span className={`ml-1.5 text-xs ${filter === f.key ? 'text-white/70' : 'text-slate-400 dark:text-slate-500'}`}>
                {f.count}
              </span>
            </button>
          ))}
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl text-sm text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all"
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          {totalCount === 0 ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center">
              <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-4">
                <UserPlus className="w-7 h-7 text-slate-400 dark:text-slate-500" />
              </div>
              <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-1">No students yet</h3>
              <p className="text-sm text-slate-400 dark:text-slate-500 max-w-xs text-center">
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">No students match the current filter</p>
              <button
              ...
              <tbody>
                {filteredStudents.map((student, index) => {
                  ...
                  return (
                    <tr
                      key={student.id}
                      className={`border-b border-slate-50 dark:border-slate-700/50 transition-all duration-500 ${isNew
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-center">
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{student.time}</span>
                      </td>
        ...
        {totalCount > 0 && (
          <div className="px-5 py-3 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <span className="text-xs text-slate-500 dark:text-slate-400">
              Showing <span className="font-bold text-slate-700 dark:text-slate-200">{filteredStudents.length}</span> of{' '}
              <span className="font-bold text-slate-700 dark:text-slate-200">{totalCount}</span> checked-in students
            </span>
